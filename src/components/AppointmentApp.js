...
class AppointmentApp extends Component {
  ...
  /**
   * Method for setting the state to the next step.
   */
  handleNext = () => {
    const {
      stepIndex
  ...
  /**
   * Method for setting the state to the previous step, if user is not in the first step.
   */
  handlePrev = () => {
    const {
      stepIndex
  ...
  /**
   * Enable and disable the next and previous button.
   */
 ...
 handleDBReponse(data) {
  ...
  renderAppointmentTimes() {
    if (!this.state.isLoading) {
      // Array with number from 0-7
      const slots = [...Array(8).keys()];
      // Go through the working hours
      return slots.map(slot => {
        ...
        const appointmentDateString =
          ...
          // Start/End time for the slot from the Array with hour shift
